...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        http.csrf().disable().httpBasic().and() // CSRF 보안에 대한 설정. 아무 설정도 하지 않으면 CSRF 보안을 하도록 설정
                ...
                .antMatchers("/","/auth/**", "/css/**", "/image/**").permitAll()//특정 경로를 지정->어떤 사용자든 접근 가능
                ...
                .loginProcessingUrl("/auth/loginProc")//로그인 Form Action URL을 지정 할 수 있다. 사용자 이름과 암호를 제출할 URL (기본값은 /login이다.)
                .defaultSuccessUrl("/")
